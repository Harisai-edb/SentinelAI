...
def monitor_disk_usage(dataframe, threshold):
    ...
    for i in range(len(dataframe)):
        if dataframe['disk_usage_percentage'][i] >= threshold:
            ...
            output_text = f"Event details: High severity event, "
            output_text += f"Disk usage at {threshold}%, {round(end_disk_usage, 2)} MB of {total_available_disk} MB Used,"
            ...
            plt.xticks(hourly_labels, rotation=90)
            plt.show()
